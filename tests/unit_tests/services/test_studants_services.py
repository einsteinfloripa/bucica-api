...
from src.models.studants_models import CadastroAlunos
...
class TestStutantService:
    tested_service = StudantService()

    test_studand = CadastroAlunos(
        id=1,
        name="teste",
        cpf="123456789",
    )

    ### ServiceResult.create_attandance_check ###
    ...
    def test_create_attendance_check_returns_service_result_ok_with_correct_entry(
        ...
        output = self.tested_service.create_attendence_check("SOME_CPF_KEY")
    ...
    def test_create_attendance_check_returns_service_result_studant_not_found_on_invalid_entry(
        ...
        output = self.tested_service.create_attendence_check("SOME_INVALID_CPF_KEY")
        assert isinstance(output.value, AppException.StudantNotFound)
    ...
    def test_get_student_returns_service_result_ok_with_correct_entry(self, mocker):
        ...
        output = self.tested_service.get_studant(entry)
